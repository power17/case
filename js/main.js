...
function p0ani1(){
  ...
  TweenLite.to(boy1,t1,{x:320-25,ease:Linear.easeNone,onComplete:p0ani2})//男孩移动完后执行第二个动画
  ...
  for (var i = 1; i < 8; i++) {
    ...
    //文字消失动画
    if(i==7){
      TweenLite.from(p1title,3,{y:p1title.y+50,alpha:0,delay:1+i*.5,ease:Back.easeOut,onComplete:p0ani3})//文字出现后执行第三个动画
    }else{
...
function p0ani3(){
  for (var i = 1; i < 8; i++) {    
    if(i==7){
      TweenLite.to(titleA[i-1],3,{x:-320,alpha:0,delay:i*0,ease:Linear.easeNone,onComplete:p0ani4})//文字飞出
    }else{
...
function p0ani4(){
  ...
  setTimeout(function(){
      ...
      $("#p0_btn")[0].addEventListener("touchstart",p0btnDown)//触发后第一个动画
